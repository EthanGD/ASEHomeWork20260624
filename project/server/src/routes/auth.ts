...
import {
  bindGithubToUser,
  buildGithubBindRedirect,
  buildGithubAuthUrl,
  buildGithubLoginRedirect,
  exchangeGithubCode,
  fetchGithubIdentityUserId,
  fetchGithubUser,
...
import {
  bindWechatToUser,
  buildWechatAuthUrl,
  buildWechatBindRedirect,
  buildWechatRedirect,
  createWechatUserAndRedirect,
  getMockWechatProfile,
  parseWechatState,
  signWechatState
  ...
  router.get("/wechat/callback", async (request, response) => {
    ...
    try {
      ...
      const userResponse = await fetch(
        `https://api.weixin.qq.com/sns/userinfo?access_token=${tokenPayload.access_token}&openid=${tokenPayload.openid}&lang=zh_CN`
      );
    ...
    try {
      ...
      if (!userId) {
        response.redirect(
          buildGithubLoginRedirect(
            undefined,
            "该 GitHub 账号未绑定系统账号，请先使用本地账号登录后在账号管理中绑定"
          )
        );
        return;
      }
