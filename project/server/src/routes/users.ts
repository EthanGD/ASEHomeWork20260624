...
export const createUsersRouter = () => {
  ...
  router.post("/", requireAuth, requirePermission("user:manage"), async (request, response) => {
    const { username, password, displayName, email, status, roleIds } = request.body as UserPayload;
  ...
  router.put(
    ...
    async (request: AuthenticatedRequest, response) => {
      ...
      const target = await query<{ auth_source: "local" | "wechat" }>(
        `SELECT auth_source FROM users WHERE id = $1 AND deleted = FALSE LIMIT 1`,
        [userId]
      );
