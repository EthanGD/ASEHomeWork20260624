...
import { AuthenticatedRequest, hasPermission, requireAuth } from "../auth.js";
...
export const createTasksRouter = () => {
  ...
  router.post("/", requireAuth, async (request: AuthenticatedRequest, response) => {
    ...
    const { title, content, status, priority, startAt, endAt } = request.body as TaskPayload;
  ...
  router.put("/:id", requireAuth, async (request: AuthenticatedRequest, response) => {
    const user = request.user!;
    ...
    const canEdit =
      hasPermission(user, "task:edit_all") ||
