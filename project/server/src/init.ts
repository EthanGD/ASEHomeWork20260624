...
export const ensureDatabase = async () => {
  ...
  const adminRole = await query<{ id: number }>(
    `
  ...
  const demoUser = await query<{ id: number }>(
    `
      INSERT INTO users(username, password_hash, display_name, email, auth_source)
      VALUES ($1, $2, $3, $4, 'local')
      ON CONFLICT (username)
  ...
  const taskCount = await query<{ count: string }>(`SELECT COUNT(*)::text AS count FROM tasks`);
