...
const rpIdFromOrigin = (origin: string) => new URL(origin).hostname;
...
const issueChallenge = (key: string, userId: number | null, origin: string) => {
...
const consumeChallenge = (key: string, expectedUserId: number | null) => {
...
const readCbor = (bytes: Uint8Array, offset: number): CborResult => {
  ...
  const readLength = (): { length: number; next: number } => {
    if (addl < 24) {
...
const decodeCborMap = (bytes: Uint8Array) => {
...
const parseAuthenticatorDataForRp = async (authenticatorData: Uint8Array, rpId: string) => {
...
const parseRegistrationAuthData = async (attestationObject: Uint8Array, rpId: string) => {
...
const coseEc2ToJwk = (coseKey: Map<unknown, unknown>) => {
...
export const createPasskeyRegisterOptions = async (
  userId: number,
...
export const verifyAndBindPasskey = async (userId: number, credential: unknown) => {
  const payload = credential as {
    id?: string;
...
export const verifyPasskeyLogin = async (credential: unknown) => {
  const payload = credential as {
    id?: string;
    response?: {
  ...
  const jwk = row.public_key_jwk as JsonWebKey;
...
const findLoginKey = (challenge?: string) => {
  if (!challenge) {
