...
export const createTranscriptionsRouter = () => {
    ...
    router.post("/", requireAuth, upload.single("file"), async (request, response) => {
        ...
        const targetUrl = `${integrations.funasrServiceUrl.replace(/\/$/, "")}${integrations.funasrApiPath.startsWith("/") ? integrations.funasrApiPath : `/${integrations.funasrApiPath}`}`;
        try {
            const form = new FormData();
            const fileBuffer = request.file.buffer;
            const fileArrayBuffer = fileBuffer.buffer.slice(fileBuffer.byteOffset, fileBuffer.byteOffset + fileBuffer.byteLength);
            form.append("file", new Blob([fileArrayBuffer], { type: request.file.mimetype }), request.file.originalname);
