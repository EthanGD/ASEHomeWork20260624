...
export const signGithubState = (payload) => jwt.sign(payload, config.jwtSecret, { expiresIn: "10m" });
export const parseGithubState = (state) => {
    if (!state) {
        return null;
    }
    try {
        const parsed = jwt.verify(state, config.jwtSecret);
...
export const buildGithubBindRedirect = (success, message) => {
...
export const buildGithubLoginRedirect = (token, error) => {
    const url = new URL(config.clientUrl);
    if (token) {
...
export const getMockGithubProfile = (state) => {
...
export const bindGithubToUser = async ({ userId, githubId, githubLogin, githubEmail }) => {
...
export const unbindGithubFromUser = async (userId) => {
    await query(`
      DELETE FROM oauth_identities
      WHERE user_id = $1 AND provider = 'github'
    `, [userId]);
};
export const fetchGithubIdentityUserId = async (githubId) => {
...
export const fetchUserIdentities = async (userId) => {
    const result = await query(`
